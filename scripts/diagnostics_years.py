...
def run_diagnostics():
    ...
    with open('diagnostics_2026.txt', 'w', encoding='utf-8') as out:
        out.write(f"{'Crop':<12} | {'Actual':<10} | {'Pred 2026':<12} | {'Pred 2024':<12}\n")
        out.write("-" * 55 + "\n")
        
        for crop in crops:
            crop_rows = df[df['Crop'] == crop]
            if crop_rows.empty: continue
            row = crop_rows.iloc[0].to_dict()
            
            def get_pred_for_year(year):
                raw_features = {
                    'Year': year, 'Month': 5,
                ...
                return model.predict(np.array([scaled_vector]))[0]
            
            pred_2026 = get_pred_for_year(2026)
            pred_2024 = get_pred_for_year(2024)
            actual = row['Yield_t_per_ha']
            
            out.write(f"{crop:<12} | {actual:<10.2f} | {pred_2026:<12.4f} | {pred_2024:<12.4f}\n")
