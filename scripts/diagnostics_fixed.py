...
def run_diagnostics():
    ...
    with open('diagnostics_fixed.txt', 'w', encoding='utf-8') as out:
        out.write(f"{'Crop':<12} | {'Actual':<10} | {'Pred (t/ha)':<12} | {'Pred (kg/ha)':<12}\n")
        out.write("-" * 55 + "\n")
        
        for crop in crops:
            # Take row 0 for Rice, row 1000 for Sugarcane etc to get different crops
            ...
            out.write(f"{crop:<12} | {actual:<10.2f} | {prediction:<12.4f} | {prediction*1000:<12.2f}\n")
